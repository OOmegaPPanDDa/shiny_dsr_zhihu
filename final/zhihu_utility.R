...
time_transform <- function(df) {
  return(ceiling((df$ans_time - df$question_time)))
}
...
get_ans_quality <- function(df) {
  ...
  good_thresh <- median(df$true_quality_score)
  
  
  
  df$quality <- ifelse(df$true_quality_score > good_thresh, "good answer","bad answer")
  return(df$quality)
}
